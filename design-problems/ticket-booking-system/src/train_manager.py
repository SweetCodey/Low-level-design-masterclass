...
class TrainManager:
    ...
    def search_trains(self, origin: str, destination: str):
        ...
        for train in self.trains.values():
            if origin in train.schedule and destination in train.schedule and \
                 train.schedule[origin][0] < train.schedule[destination][0]:
                trains.append(train)
        return trains
...
class Train:
    def __init__(self, train_id: int, train_name: str, schedule: dict[str: (int, datetime.time)],
                 seats: dict[TrainSeatType: int]):
    ...
    def add_seats(self, seats: dict[TrainSeatType: int]):
        ...
        for seat_type, count in seats.items():
            for i in range(count):
                train_seats.append(TrainSeat(len(train_seats) + 1, self.train_id, seat_type))
        return train_seats
